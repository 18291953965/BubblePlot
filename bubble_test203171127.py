...
# 这里的data是三原色
#data = cm.rainbow(np.random.rand(N));
#cm = plt.cm.rainbow;
# cmap = plt.cm.rainbow
...
bubble = ax.scatter(x= Prod_Category.Sales[Prod_Category.Category == '办公用品'],
           ...
           cmap=cm, alpha = 0.7,label = 'displacement velocity(mm/month)');
...
#ax.set_ticks([np.min(data), 0.25,0.5, 0.75, np.max(data)])
...
ax.set_label('Change in reservoir level(m/month)')
